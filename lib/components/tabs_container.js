  ...
  renderFloatingActionButtons: function renderFloatingActionButtons() {
    ...
    if (activeItem) {
      if (isActiveItemAFile) {
        ...
        floatingActionButton = <ToolBarButton onClick={this.props.app.toggleItemPane.bind(this.props.app, activeItem)} title={'Move to ' + (activeItem.pane === 'bottom' ? 'top' : 'bottom')} className={activeItem.pane === 'bottom' ? 'fa-arrow-up' : 'fa-arrow-down'} />;
      }
    }

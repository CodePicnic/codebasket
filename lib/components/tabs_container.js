...
TabsContainer = React.createClass({
  ...
  renderOption: function renderOption(option, index) {
    ...
    return (
      <a key={index} className={'console-option-' + optionIdentifier} target={option.target || null} href={option.href || null} onClick={option.action}>
  ...
  render: function render() {
    ...
    return (
      <aside className="console-tabs-container">
        <nav className={'console-tabs' + (this.props.parentView.state.isFullScreen ? ' collapsed' : '')}>
        ...
        <section className={'console-editor-overlay' + (isActiveItemAFile ? ' enabled' : '') + (this.props.parentView.state.isFullScreen ? ' full-screen' : '')}>
          <ToolBarButton onClick={this.props.parentView.toggleFullScreen} title="Toggle full screen" className={'with-border ' + (this.props.parentView.state.isFullScreen ? 'fa-compress' : 'fa-expand')} />
        </section>
      </aside>
    );
  }

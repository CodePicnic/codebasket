...
TabsContainer = React.createClass({
  ...
  renderTabPage: function(item, index) {
    if (item.type === 'file') {}
      ...
      return (
        <iframe ref={'tabpage-' + item.id} className={'console-tabpage' + (item.isActive ? ' active' : '')} data-identifier={item.id} title="Press enter to submit commands" src={item.location + '?id=' + item.id + '&console_id=' + item.consoleId} key={index}></iframe>
      );
    }
    else {
      return (
    ...
    return (
      <aside className="console-tabs-container">
        <nav className="console-tabs">
          ...
          {tabsList}
          <ToolBarButton onClick={this.toggleModal} id="add-tab" title="Add tab" className={'fa-plus' + (this.state.isModalVisible ? ' active' : '')} />

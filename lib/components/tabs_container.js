...
TabsContainer = React.createClass({
  ...
  renderTabStrip: function renderTabStrip() {
    var codeBasket = this.props.app,
        simpleLayout = this.props.simple,
  ...
  render: function render() {
    ...
    return (
      <aside className="console-tabs-container">
        ...
        <section className={'console-editor-overlay' + (isActiveItemAFile ? ' enabled' : '') + (isFullScreen ? ' full-screen' : '')}>
          <ToolBarButton onClick={this.props.parentView.toggleFullScreen} title="Toggle full screen" className={'with-border ' + (isFullScreen ? 'fa-compress' : 'fa-expand')} />
        </section>
      </aside>
    );
  }

...
TabsContainer = React.createClass({
  ...
  renderTab: function(item, index) {
    ...
    return (
      <a href="#" className={'console-tab' + (item.isCloseable ? ' closeable' : '') + (item.isActive ? ' active' : '')} onClick={this.onTabClick.bind(null, item, index)} data-mode={item.language} key={index}>
  ...
  render: function() {
    var activeItem = this.props.items.find(function(item) { return item.isActive; }),
    ...
    var isSidebarVisible = this.props.parentView.state.isSidebarVisible,
        toolbarOptions = this.props.app.toolbarOptions.map(function(toolbarOption, index) {
          return (
            <ToolBarButton onClick={toolbarOption.onClick} title={toolbarOption.title} className={toolbarOption.icon} key={index} />
          );
        }),

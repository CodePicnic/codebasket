...
Terminal = React.createClass({
  ...
  render: function terminalRender() {
    var item = this.props.item,
        ...
        queryStringSeparator = item.location && (item.location.match('\\?') ? '&' : '?'),
        src = item.location ? (item.location + queryStringSeparator + 'id=' + item.id + '&console_id=' + item.consoleId) : 'about:blank';

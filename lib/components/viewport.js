...
Viewport = React.createClass({
  ...
  calculateNavBarWidthExcess: function() {
    ...
    if (global.matchMedia && global.matchMedia(mediaQueryBreakpoint).matches) {
      return ((instance.actions.length + 2 + Number(!!instance.info)) * 48 );
    }
    else {
      return ((instance.actions.length + 2 + Number(!!instance.info)) * 33);
    }
  },
  ...
  renderMobileNavBarItem: function renderMobileNavBarItem(item, index) {
    ...
    return (
      <li className='navbar-tab' key={index}>
        {closeButton}
        <span title={item.name} className='navbar-tab-title' onClick={this.onClickTab.bind(this, item, index)}>{item.name}</span>
      </li>
    );
  },
  renderMobileNavBar: function renderMobileNavBar() {
    ...
    return (
      <nav ref="navbar" className="codebasket-navbar touch" style={{ width: 'calc(100% - ' + navBarWidthExcess + 'px)' }}>
        <div className='nabvar-placeholder'>
          {closeButton}
          <span className='navbar-tab-title' onClick={this.toggleExtraTabs}>{activeItem && activeItem.name}</span>

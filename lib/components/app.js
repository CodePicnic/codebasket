...
App = React.createClass({
  ...
  renderTabsContainer: function renderTabsContainer() {
    ...
    if (itemsInBottomPane.length === 0) {
      return <TabsContainer items={itemsInTopPane} app={codeBasket} parentView={this} />;
    }
    else {
      return (
        <SplitPane split="horizontal" minSize={200} defaultSize={200}>
          <TabsContainer items={itemsInTopPane} app={codeBasket} parentView={this} />
  ...
  render: function() {
    var codeBasket = this.props.codeBasket,
        ...
        brand = codeBasket.brand ? <Brand brand={codeBasket.brand} /> : undefined,

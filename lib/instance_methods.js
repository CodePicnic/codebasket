...
function createFileSession(item) {
  if (item.type === 'file') {
    if (item.session) {
      ...
      var sessionEvent = global.document.createEvent('CustomEvent');
      sessionEvent.initCustomEvent('codebasket:addsession', true, true, {
        codeBasket: this,
        item: item
      });

      global.dispatchEvent(sessionEvent);
...
function selectItem(item, index) {
  ...
  if (activeItem !== item) {
    ...
    if (item.type === 'file' && item.session && this.getCurrentEditor()) {
      this.getCurrentEditor().setSession(item.session);
...
function moveItemToPane(item) {
  ...
  if (activeFile) {
    if (activeFile.session && this.getCurrentEditor()) {
      var self = this;
      ...
      if (global.requestAnimationFrame || global.webkitRequestAnimationFrame) {
        (global.requestAnimationFrame || global.webkitRequestAnimationFrame)(function() {
          self.getCurrentEditor().setSession(activeFile.session);
        });
      }
      else {
...
function addFileToSidebar(fileName, fullName, rootPath) {
  ...
  this.addSidebarItems(newSidebarItem, rootPath);
}

function addFolderToSidebar(folderName, fullName, rootPath) {
  var newSidebarItem = {};
...
function getBrowser() {
  return find(this.items, function(item) { return item.type === 'browser'; });
}

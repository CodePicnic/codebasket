...
TabsContainer = React.createClass({
  ...
  renderTab: function renderTab(item, index) {
    ...
    if (item.isCloseable) {
      closeButton = (
        <small className="console-tab-close" title="Close" onClick={this.closeTab.bind(null, item, index)}><i className="icon icon-close-2"></i></small>
      );
    }
    ...
    if (option.action) {
      return (
        <span key={index} className={'console-option-' + optionIdentifier} onClick={this.closeOptionsList.bind(this, option.action)}>
          <i className={'fa icon ' + option.icon}></i>{option.title}
        </span>
      );
    }
    else {
  ...
  renderAddTabButton: function renderAddTabButton() {
    return (
      <ToolBarButton onClick={this.props.app.uiOptions.addAction || this.toggleModal} id="add-tab" title="Add tab" className={'with-left-border icon-add' + (this.state.isModalVisible ? ' active' : '')} />
    );
  },
  renderOptionsButton: function renderOptionsButton() {
    return (
      <ToolBarButton onClick={this.toggleOptions} title="Options" className={'with-caret icon-cog' + (this.state.isOptionsListVisible ? ' active' : '')} />
    );
  },
  ...
  renderTabStrip: function renderTabStrip() {
    ...
    if (inSimpleMode) {
      ...
      return (
        <nav className={'console-tabs' + (isFullScreen ? ' collapsed' : '')}>
          <ToolBarButton onClick={this.props.parentView.toggleSidebar} title="Toggle sidebar" className={'with-border ' + (isSidebarVisible ? 'icon-hide-sidebar' : 'icon-show-sidebar')} id="toggle-sidebar" />
          {tabsList}
          {addTab}
  ...
  renderFloatingActionButtons: function renderFloatingActionButtons() {
    ...
    if (activeItem) {
      if (isActiveItemAFile) {
        floatingActionButton = <ToolBarButton onClick={this.props.parentView.toggleFullScreen} title="Toggle full screen" className={isFullScreen ? 'icon-compress' : 'icon-expand'} />;
        otherFloatingActionButtons = floatingActionButtonsForFiles.map(this.renderFloatingActionButton);
      }
      else {
        floatingActionButton = <ToolBarButton onClick={this.props.app.moveItemToPane.bind(this.props.app, activeItem)} title={'Move to ' + (activeItem.pane === 'bottom' ? 'top' : 'bottom')} className={activeItem.pane === 'bottom' ? 'icon-arrow-up' : 'icon-arrow-down'} />;
